...
class ChannelPackageSyncTests(unittest.TestCase):
    ...
    def test_remote_resources_copy_full_source_aa_before_final_overlay(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (final_root / "Bundle" / "Android" / "shared.bundle").write_text("new bundle", encoding="utf-8")
            (final_root / "Bundle" / "catalog.json").write_text("new catalog", encoding="utf-8")
    ...
    def test_without_remote_resources_only_final_result_is_copied(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (source_aa / "settings.json").write_text("source settings", encoding="utf-8")
            (final_root / "Bundle" / "Android").mkdir(parents=True)
            (final_root / "Bundle" / "Android" / "changed.bundle").write_text("changed", encoding="utf-8")
