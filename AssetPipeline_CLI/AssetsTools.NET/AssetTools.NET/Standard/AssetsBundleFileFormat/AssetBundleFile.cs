...
namespace AssetsTools.NET
{
    public class AssetBundleFile
    {
        ...
        public void Pack(AssetsFileWriter writer, AssetBundleCompressionType compType,
            ...
            AssetBundleFSHeaderFlags? layoutFlags = null)
        {
            if (Header == null)
                throw new Exception("Header must be loaded! (Did you forget to call bundle.Read?)");
            ...
            AssetBundleFSHeaderFlags preservedLayoutFlags = layoutFlags.HasValue
                ? layoutFlags.Value & (AssetBundleFSHeaderFlags.OldWebPluginCompatibility | AssetBundleFSHeaderFlags.BlockInfoNeedPaddingAtStart)
                : AssetBundleFSHeaderFlags.None;
            AssetBundleFSHeaderFlags compressionFlag = layoutFlags.HasValue
                ? layoutFlags.Value & AssetBundleFSHeaderFlags.CompressionMask
                : compType switch
                {
                    AssetBundleCompressionType.LZMA => AssetBundleFSHeaderFlags.LZ4HCCompressed,
                    AssetBundleCompressionType.LZ4 => AssetBundleFSHeaderFlags.LZ4HCCompressed,
                    AssetBundleCompressionType.LZ4Fast => AssetBundleFSHeaderFlags.LZ4Compressed,
                    _ => AssetBundleFSHeaderFlags.None
                };
            ...
            AssetBundleFSHeader newFsHeader = new AssetBundleFSHeader
            {
                ...
                CompressedSize = 0,
                DecompressedSize = 0,
                Flags = compressionFlag | AssetBundleFSHeaderFlags.HasDirectoryInfo | preservedLayoutFlags |
                ...
                case AssetBundleCompressionType.LZ4Fast:
                {
                    ...
                    while (uncompressedBlock.Length != 0)
                    {
                        ...
                        else
                        {
                            ...
                            ushort blockCompressionFlag = compType == AssetBundleCompressionType.LZ4Fast ? (ushort)0x02 : (ushort)0x03;
                            AssetBundleBlockInfo blockInfo = new AssetBundleBlockInfo()
                            {
                                CompressedSize = (uint)compressedBlock.Length,
                                DecompressedSize = (uint)uncompressedBlock.Length,
                                Flags = blockCompressionFlag
                            };
            ...
            byte[] bundleInfoBytesCom;
            switch (compressionFlag & AssetBundleFSHeaderFlags.CompressionMask)
            {
                case AssetBundleFSHeaderFlags.None:
                    bundleInfoBytesCom = bundleInfoBytes;
                    break;
                case AssetBundleFSHeaderFlags.LZMACompressed:
                    using (MemoryStream compressedInfoStream = new MemoryStream())
                    {
                        using MemoryStream infoStream = new MemoryStream(bundleInfoBytes);
                        SevenZipHelper.Compress(infoStream, compressedInfoStream, null);
                        bundleInfoBytesCom = compressedInfoStream.ToArray();
                    }
                    break;
                case AssetBundleFSHeaderFlags.LZ4Compressed:
                    bundleInfoBytesCom = LZ4Codec.Encode32(bundleInfoBytes, 0, bundleInfoBytes.Length);
                    break;
                case AssetBundleFSHeaderFlags.LZ4HCCompressed:
                    bundleInfoBytesCom = LZ4Codec.Encode32HC(bundleInfoBytes, 0, bundleInfoBytes.Length);
                    break;
                default:
                    throw new NotSupportedException($"Unsupported UnityFS block info compression: {compressionFlag & AssetBundleFSHeaderFlags.CompressionMask}");
            }

            long bundleInfoPadding = 0;
            if (!blockDirAtEnd && (newFsHeader.Flags & AssetBundleFSHeaderFlags.BlockInfoNeedPaddingAtStart) != 0)
            {
                long dataStartWithoutPadding = headerSize + bundleInfoBytesCom.Length;
                bundleInfoPadding = ((dataStartWithoutPadding + 15) & ~15) - dataStartWithoutPadding;
            }

            long totalFileSize = headerSize + bundleInfoBytesCom.Length + bundleInfoPadding + totalCompressedSize;
            ...
            if (!blockDirAtEnd)
            {
                ...
                if ((newFsHeader.Flags & AssetBundleFSHeaderFlags.BlockInfoNeedPaddingAtStart) != 0)
                    writer.Align16();
